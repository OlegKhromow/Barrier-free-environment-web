...
@Component({
  selector: 'app-pending-location-info',
  imports: [
    DecimalPipe,
    NgClass,
    SlideshowComponent
  ],
  templateUrl: './pending-location-info.component.html',
  styleUrl: './pending-location-info.component.css',
})
export class PendingLocationInfoComponent implements OnChanges {
  ...
  loadCommentImages(check: any, imageServiceId: string) {
    ...
    const combinedId = `${this.pendingLocation?.id}_${imageServiceId}`;
